...
function isContrastState(value: unknown): value is ContrastState {
  ...
  const v = value as { fgColor?: unknown; bgColor?: unknown; combinations?: unknown; label?: unknown };
  ...
  for (const combination of v.combinations) {
    ...
    const c = combination as { fg: unknown; bg: unknown; ratio: unknown; passAA?: unknown; passAAA?: unknown };
    if (typeof c.fg !== 'string' || typeof c.bg !== 'string' || typeof c.ratio !== 'number') return false;
...
export const contrastTool: Tool<ContrastState> = {
  ...
  serialize: (state) => JSON.stringify({ fgColor: state.fgColor, bgColor: state.bgColor, combinations: state.combinations, label: state.label }, null, 2),
  deserialize: (data) => {
    if (isContrastState(data)) {
      return {
        success: true,
        data: {
    ...
    return {
      success: false,
      error: 'Invalid data format: expected { fgColor: string, bgColor: string, combinations: Array<{ fg, bg, ratio, passAA, passAAA }>, label?: string }',
    };
  },
  exporters: [
